...
# Prepare the data ####
data=read_xlsx('Vernal Midnight Data.xlsx')
data=data[1:8]
names(data)=c('date', 's1', 's2', 's3', 's4', 's5', 's6', 's7')
...
sm1=data$s1/100 # Soil moisture content at sensor 1
sm2=data$s2/100 # Soil moisture content at sensor 2
sm3=data$s3/100 # ...
sm4=data$s4/100
sm5=data$s5/100
sm6=data$s6/100
...
ET.B.2=c()
for(i in 1:n) { 
  ET.B.2[i]=(sm2[i]-sm2[i+1])*(z23-z12)}
for(i in 1:(n-1)) { # eliminate negative ET values
...
ET.B.3=c()
for(i in 1:n) { 
  ET.B.3[i]=(sm3[i]-sm3[i+1])*(z34-z23) }
for(i in 1:(n-1)) { # eliminate negative ET values
...
ET.B.4=c()
for(i in 1:n) { 
  ET.B.4[i]=(sm4[i]-sm4[i+1])*(z45-z34) }
for(i in 1:(n-1)) { # eliminate negative ET values
...
ET.B.5=c()
for(i in 1:n) { 
  ET.B.5[i]=(sm5[i]-sm5[i+1])*(z56-z45) }
for(i in 1:(n-1)) { # eliminate negative ET values
  if (ET.B.5[i]<0) {ET.B.5[i]=0} }         
plot(date, ET.B.5, 
     type='h', lwd=2,
...
ET.B.6=c()
for(i in 1:n) { 
  ET.B.6[i]=(sm6[i]-sm6[i+1])*(z67-z56) }
for(i in 1:(n-1)) { # eliminate negative ET values
  if (ET.B.6[i]<0) {ET.B.6[i]=0} }         
plot(date, ET.B.6, 
     type='h', lwd=2,
...
ET.B.7=c()
for(i in 1:n) { 
  ET.B.7[i]=(sm7[i]-sm7[i+1])*(z7-z67)*2 }
for(i in 1:(n-1)) { # eliminate negative ET values
  if (ET.B.7[i]<0) {ET.B.7[i]=0} }  

ET.B.7[79]=NA # Day 79: ET=49mm - probably irrigation data
...
ET.B[is.na(ET.B)]=0
for(i in 1:(n-1)) { # eliminate ET>20mm/day
  if (ET.B[i]>20) {ET.B[i]=0}}         
ET.B[ET.B==0]=NA

plot(date, ET.B, 
     type='h', col='darkslategray3', lwd=2, ylim=c(0,20), 
...
ET.C.7=c()
for(i in 1:n) { 
  ET.C.7[i]=(sm7[i]-sm7[i+1])*(zr-z67) }
for(i in 1:(n-1)) { # eliminate negative ET values
  if (ET.C.7[i]<0) {ET.C.7[i]=0} }  

# ET.C.7[79]=NA # Day 79: ET=49mm - probably irrigation data
...
plot(D.df$Date, D.df$ET,
     type='h', lwd=2, col='darkslategray3', ylim=c(0,20),
